...
class EpsilonGreedyAgent():
    ...
    def run(self):
        self.env = EnvWithMemory(retro.make(game=config.GAME, state=config.STATE, info=config.INFO,
    ...
    def _random_movement(self):
        # B is using weapon, A is jumping (disabled for now)
        # ["B", null, "SELECT", "START", "UP", "DOWN", "LEFT", "RIGHT", "A"]
        action = [numpy.random.randint(0, 2), 0, 0, 0, numpy.random.randint(0, 2), numpy.random.randint(0, 2),
                  numpy.random.randint(0, 2), numpy.random.randint(0, 2), 0]
        observation, rewards, done, info = self.env.step(action)
