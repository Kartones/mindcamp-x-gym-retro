...
EPSILON = 0.005  # % of exploration. consider each level has thousands of steps
...
class EpsilonGreedyAgent():
    ...
    def move_step(self, last_info):
        if self.env.current_iteration > 0:
            ...
            return self._ai_movement(last_info)
    ...
    def _epsilon_greedy_movement(self, last_info):
        ...
        if probability < EPSILON:
            ...
            if self.env.current_time < self.best_time:
                ...
                # We reached end of previous list of actions, try to play smart
                return self._ai_movement(last_info)
    ...
    def _random_movement(self):
        ...
        action = [0, 0, 0, 0, 0, 0, numpy.random.randint(0, 2), numpy.random.randint(0, 2), self.env.current_time % 2]
        observation, rewards, done, info = self.env.step(action)

        return observation, rewards, done, info

    def _ai_movement(self, last_info):
        # gb screen size is 160x144
        # left border is 8px
        # right border is 40px
        # play area is 112px (from pixel 8 to pixel 120)
        # but after debugging using integration GUI, ball counts from 16 to 123
        # and player paddle also goes from 16 to 120
        paddle_size = 24
        ...
        if last_info:
            ...
            # just the first step/action
            go_left = False
            go_right = False

        # Buttons for GameBoy games defined at gym-retro/retro/cores/gambatte.json
        # ["B", null, "SELECT", "START", "UP", "DOWN", "LEFT", "RIGHT", "A"]
        # Note: Ball only goes out when pressing A, and if kept pressed will always stay that way
        # so we constantly press and release the button
        action = [0, 0, 0, 0, 0, 0, go_left, go_right, self.env.current_time % 2]
